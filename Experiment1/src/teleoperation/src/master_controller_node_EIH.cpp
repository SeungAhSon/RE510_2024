...
public:
	...
	void MasterDevStateCallback(const virtual_master_device::master_dev_stateConstPtr &msg){
        ...
        if(teleoperation_mode_ == 1){
            ...
            master_command.pose.orientation.x = lowpass_filter(master_command.pose.orientation.x, rotation_difference.getZ());
            master_command.pose.orientation.y = lowpass_filter(master_command.pose.orientation.y, -rotation_difference.getX());
        ...
        // 2.Position to Velocity : publish the position command
            ...
            // Orientation
            master_command.pose.orientation.x = lowpass_filter(master_command.pose.orientation.x, m_rot.z());
            master_command.pose.orientation.y = lowpass_filter(master_command.pose.orientation.y, -m_rot.x());
